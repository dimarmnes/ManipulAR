...
MorseSpeaker::MorseSpeaker(int t_spkrPin)
{
  ...
  sideToneOn = false;
  outputToneOn = false;
  outputTonePitch = OUTPUT_TONE_PITCH;
  sideTonePitch = SIDE_TONE_PITCH;
  
  noTone(spkrOut);
}

void MorseSpeaker::setOutputTonePitch(unsigned int pitch)
{
  outputTonePitch = pitch;
}

void MorseSpeaker::setSideTonePitch(unsigned int pitch)
{
  sideTonePitch = pitch;
}
